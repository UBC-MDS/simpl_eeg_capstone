...
For the unsupervised clustering method, we have tried both KMeans clustering model and Hidden Markov model to identify potential brain states in the data across different time periods. Brain states are considered as similar patterns of 19 electrodes over a period of time in the raw EEG data. The KMeans clustering method utilizes the [KMeans](https://scikit-learn.org/stable/modules/generated/sklearn.cluster.KMeans.html) model from [Scikit-learn](https://scikit-learn.org/stable/index.html) to separate data from each time periods into different clusters based on its distance from the center of the entire dataset. The Hidden Markov model method uses the GaussianHMM model from [hmmlearn](https://hmmlearn.readthedocs.io/en/latest/) to learn the Gaussian distribution of the EEG signals and update the transmission probability matrix based on the changes of the distribution as it transitions from one time section to another. Furthermore, we looked into expanding our analysis to methods beyond what we covered in MDS such as Self Organizing Maps (SOM) and K-means and Dynamic Time Warping using `tslean`. 
...
Jupyter Notebooks So as a stretch goal we decided to include an unsupervised machine learning script to identify and cluster patterns in the EEG data. The goal is look for different brain states within the same cluster and to distinguish structures of node signals and patterns in the brain indicating the effect of an impact at specific timestamps. Brain states are activities of the brain. Here we can see groups of neurons firing at the same time in the same frequency creating different states. (Include image from presentation). Since it is unsupervised approach, we do not have target values which make it difficult to objectively measure the effectiveness of the algorithms. In fact, there no definitive approach. 

Today, scientists and labs who are dealing with EEG data use advanced algorithms and models such as Multi-Variate Auto-Regressive or use intense simulations models to find the optimal value of K in other word the number of clustering groups. For now, it’s extremely difficult to define brain states in order to cluster them probably. Having said that, we have used methods that we learned in MDS to evaluate and justify the K values we choose. 

We took two directions to solve this problem. Analyzing the data set as single stimestamps and range of timestamps. We have used diffrent methods in order to identify the optimal number of clusters and patterns. 

K-Means Algorithm: 

- Elbow method

With the Elbow method we can test and evaluate range of K value and the algortihm will detarmine the optimal value of K. This method looks at the sum inertia and its distances, the inertia decreases as K increases.

- Silhouette method

This method is not dependent on cluster centers. It uses the mean intra-cluster distance and mean nearest cluster distance for each sample. The Silhouette score represent how well separated the clusters are. 
