...
To better assist the partner to further explore the clustering task and extend the work, we have created a Jupyter notebook file for each approaches we have tried so far, namely `KMeans` and `Hidden Markov Model`, and included them as sub-sections under the `Clustering` chapter in the [Jupyter book documentation](https://ubc-mds.github.io/simpl_eeg_capstone/clustering.html). Inside each notebooks, we have outlined a detailed workflow from data preprocessing, model fitting to output visualization for each approaches. We have also documented our reasons for choosing the approach, general introduction of the approach, explanation of key components and interpretation of model output, which would make this notebook serve as a good reference point for partner to further explore the task in the future. Additionally, we included a section called `Next steps` in some applicable notebooks to let the partner know that there are some potentially useful steps in finding the significant results but we haven't had time to implement yet, such as using rolling means and sliding windows as feature engineering.
