...
1) A Python package for generating advanced EEG visualizations. The package contains 6 different modules for visualizing EEG data, with a variety of types of plots to view the data from different perspectives. The package is built in an editable way so that the partner may continue to develop and customize the functionality after the Capstone project is coplete. 

2) An interactive web application to provide a user interface for the package. The web application provides a user interface (UI) for the package to allow people to use the package functionality without needing to write any code. The UI also benefits from having all visualization options in one place, and is a useful tool for exploring what the package has to offer. 
...
1) **Raw voltage plot** for visualizing raw voltage value changes over time for each node. This plot allows you to see the most data all in one place, and represents time on the x-axis so it is not animated. When using the MNE package directly an interactive version of the plot is available. 

![Raw voltage plot](images/raw_voltage.png)

2) **2D topographic head map** for visualizing a topographic heatmap of the voltage values mapped to a 2D model of a head. This allows the user to see where the changes are occurring on the scalp, with the animation showing how things change over time. One downside is that some distortion occurs when representing a 3D head in two dimensions, making the edges of the head appear to take up the same amount of space as the center of the head which is not reflective of reality. However, this is still one of the most useful visualizations due to it's simplicity and quick rendering time.   

![2D topographic head map](images/2d_head.png){ height=20% }

3) **3D head map** for visualizing a topographic heatmap of voltage values mapped to a 3D model of skull. The 3D representation allows the user to see how the voltage changes look on a more accurate representation of the skull than the 2D head map, and can be rotated to see the head from multiple perspectives. However, it is slower to render than the 2D head map because interpolation must be run on every frame to generate the figure. 

![3D topographic head map](images/3d_head){ height=20% }

\newpage

4) **3D brain map** for visualizing a topographic heatmap of interpolated voltage values mapped to their presumed position on a 3D model of a brain. This allows the user to view where the source of the voltage changes may be emanating from. MNE provides the calculations for converting location on the skull to location on the brain, which is more complicated than a direct mapping based on location. 
